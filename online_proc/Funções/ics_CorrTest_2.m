function [xfinal, tElapsed, VectorClasses] = ics_CorrTest_2(x, cl, noise, Srate, outfile, thrd, method)
...
% VectorClasses     -   Vector of index classes (matrix confusion)
...
%% Process variables (Time processing, ILD, CrossPSD)
tElapsed = zeros(Nframes,1);
VectorClasses = zeros(Nframes,1);
...
%     disp(['x= ', num2str(size(x(k:k+len-1,1)))])
    ...
    %% Obtain ILD mean per frame
    % Calculate the level diference by: L(b,k) = |Al|^2 - |Ar|^2
    %  ----------- Get average value of ILD ----------- 
	ILDMean = mean(20.*log10((sig2./sig1)));
    
    %  -----------  Get cross-PSD estimation ---------- 
%     [CORR, ~] = xcorr(sig2, sig1);
%     [ACORR_L, ~] = xcorr(sig1, sig1);
%     [ACORR_R, ~] = xcorr(sig2, sig2);
%     CORR = (1/Nframes).*CORR;
%     ACORR_L = (1/Nframes).*ACORR_L;
%     ACORR_R = (1/Nframes).*ACORR_R;
%     
%     CORR_L = CORR./ACORR_L;
%     CORR_R = CORR./ACORR_R;    
%     
%     % Mean value per frames
%     CORR_LMean = mean(CORR_L);
%     CORR_RMean = mean(CORR_R);
    
    
    % Based on variables 'ILDMean', 'CORR_LMean' and 'CORR_RMean' identify
    % which channel has the hightest noise power
%     if (ILDMean < 0 && (CORR_LMean < CORR_RMean))
%     if (CORR_LMean < CORR_RMean)        
	if (ILDMean < 0 )
        
	
        
%         disp('Left noise higher')
        
        % Index classe vector (use in confusion matrix)
        VectorClasses(i,1) = 0;

        %% Estimate noise spectrum (channel 2)
        % use noise estimation algorithm
        if i==1
            % Intialization related to channel 1
            ...
            % Intialization related to channel 2
...
%         ksi_IEC = cl_sig1./sig1;  % True instantaneous SNR        
...
% figure;
% plot(ksi_IEC,'k');hold on
% plot(ksi_IEC2,'r')
% 
% figure
% plot(ksi_iter1,'k'); hold on
% plot(ksi_iter2,'r')
