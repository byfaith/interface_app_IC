function p = initialize_ACE_mod(flagUserMap)
...
if (isfield(p,'Left')==1)
    ...
    % 	p.Left.channel_stim_rate = p.Left.analysis_rate;
    p.Left.block_shift = ceil(fs/p.Left.StimulationRate);
    ...
    % ncyclesL = uint16(ceil(p.Left.interpulseDuration/0.1)); % + 0.5);
    p.Left.ncycles = uint16((p.Left.interpulseDuration/0.1) + 0.5);
...
if (isfield(p,'Right')==1)
    ...
    % ncyclesR = uint16(ceil(p.Right.interpulseDuration/0.1)); % + 0.5);
    p.Right.ncycles = uint16((p.Right.interpulseDuration/0.1) + 0.5);
