...
%
% Modified by Gustavo Mourao 02/03/2018
...
% size_y1 = size(y1,1);
size_y1 = size(hue_signal_new,1);
% len_y1 = length(y1(2,:));
len_y1 = length(hue_signal_new(2,:));

%%% ------------- ACE STRATEGY  ------------- 
Nmax = 8; % Number of maximum activated electrodes, per frame
for k=1:size(hue_signal_new,2)
    [~, idx] = sort(hue_signal_new(:,k),'descend');
    
    for p=Nmax+1:length(idx)
        hue_signal_new(idx(p),k) = 0;
    end
    
end
% disp('Decompose values in terms of max values')
% size(temp_hue_signal_new)
% temp_hue_signal_new
% idx
% figure;imagesc(hue_signal_new)
%%% ------------- ACE STRATEGY  ------------- 
...
rms_y_channels = sqrt(mean(hue_signal_new.^2,2));
...
xlabel('Trechos')
ylabel('Canais')
save('noisySpeech_spec.mat','hue_signal_new')
